    ...
    if config["donateRole"] != "none":
        with open(donationDB, 'rt') as f:
            ...
            if updated == True:
                ...
                dRole = await utilities.getRole(message, config["donateRole"])

                if dRole == 0:
                    dRole = await client.create_role(message.server, permissions=discord.Permissions.none(), name=config["donateRole"])
            ...
            await client.send_message(message.author, "To donate to `%s`, send an amount greater than $`%s` to: ```%s```\nThank you :slight_smile:" % (message.server, config["donateAmount"], donateAddress))
    else:
        await client.send_message(message.author, "`%s` isn't accepting donations at the moment." % (message.server))
...
async def Donations(client, message):
    ...
    counter = 0
    maxCounter = 5 if len(message.content) < 8 else message.content[7:]
